    ...
    <div class="col s12">
      <div class="card">
        <div class="card-content">
          <h4 class="card-title">Pricing View</h4>
          <div class="row">
            <div class="col s12">
              <div id="page-length-option_wrapper" class="dataTables_wrapper"><table id="page-length-option" class="display dataTable dtr-inline collapsed" role="grid" aria-describedby="page-length-option_info" style="width: 100%;">
                ...
                <tbody>
					<?php
				$url="http://www.softwaredemosite.in/api/fetch_all_pricing.php";
				//$url="http://localhost/laundry/api/fetch_all_pricing.php";
					//  Initiate curl
					$ch = curl_init();
                    ...
                    <td><?php echo $item['product_process'];?></td>                   
...
<!-- Modal Structure -->
  ...
  <form method="post" id="priceedit">
  <div class="modal-header" style="text-align:center;">
  <h5>Price View/Edit</h5>
  </div>
    <div class="modal-content">      
	  ...
	  <input type="hidden" name="id" id="id">
          ...
          <label for="product_id">Product Id</label>
		   ...
		   <select name="product_process" id="product_process" required>		   
...
<!-- END RIGHT SIDEBAR NAV -->
 ...
 $(document).ready(function() {
    ...
    var id = $(this).attr('id');   		
			$.ajax({
			url:"api/fetch_single_pricing_id.php?id="+id,
			method:"POST",
			//data:{id:fullname},
			dataType:"json",
			...
			{					
				...
				$('#id').val(data['id']);
				$('#product_cost').siblings('label').addClass('active');
				$('#product_cost').val(data['product_cost']);
				$('#product_process').prepend("<option selected value="+data['product_process']+">"+data['product_process']+"</option>")
				$('#product_process').formSelect();	
	...
	$('#btnDelete').click(function(){
		...
		{
				$.ajax({
				url: "api/delete_pricing.php?id="+cusid,
				method: "POST",
		...
		return false
	 	...
	 	$('#priceedit').on('submit', function(event){			
